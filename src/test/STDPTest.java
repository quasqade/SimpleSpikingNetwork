 ...
 * constant rate and multiple presynaptic neurons firing at different delays with respect to
 * postsynaptic neuron. The various testing results are then represented visually.
 */
public class STDPTest {
  ...
  public static void main(String[] args) {
    ...
    final IzhikevichParameters izhikevichParameters = new IzhikevichParameters(0.02, 0.2, -65, 2,
        30, 0.1);
    final IzhikevichIC izhikevichIC = new IzhikevichIC(-65, -13, 0); //ICs as defined by Izhikevich

    Neuron postsynaptic = new Neuron(Neuron.NeuronType.IZHIKEVICH,
        5); //postsynaptic neuron that will be firing constantly.
    ...
    SwingUtilities.invokeLater(new Runnable() {
      @Override
      public void run() {
        new LineChartFrame(postSynapticVoltage, "Postsynaptic voltage", "Time (ms)",
            "Voltage (mV)");
      }
    });
    ...
    SwingUtilities.invokeLater(new Runnable() {
      @Override
      public void run() {
        new XYChartFrame(finalWeightSeries, "Final weights vs timing difference",
            "t_post - t_pre (ms)", "Final weight", XYChartFrame.MarkerType.SMALL_CROSS);
      }
    });
  }
}
